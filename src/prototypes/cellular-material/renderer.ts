...
export function renderDebug(root: HTMLElement, world: World, selected?: number) {
  ...
  q('cm-trace').innerHTML = `<p><b>${msg(`cell.${trace.material}`)} · Tick ${trace.tick}</b> · ${coords(trace.from)} → ${coords(trace.to)}</p><ol>${trace.checks.map(c => `<li>${msg(`cell.${c.direction}`)}: ${msg(`cell.${c.result}`)}${c.roll === undefined ? '' : ` · ${msg('cell.roll')}: ${c.roll.toFixed(6)} / ${msg('cell.threshold')}: ${FIRE_RULES.ignitionProbability}`}</li>`).join('')}</ol><p><b>${msg('cell.chosen')}: ${msg(`cell.${trace.chosen}`)}</b>${trace.life === undefined ? '' : ` · ${msg('cell.life')}: ${trace.life}`}</p>`;
}
