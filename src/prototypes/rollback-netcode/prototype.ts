...
export function mountRollback(container: HTMLElement): () => void {
  container.innerHTML = `<main class="prototype-page shell rb-page">
    ...
    <section class="rb-box" data-rb-aria="rollback.controls">
      ...
      <p class="rb-muted">${text('rollback.lossHelp')}</p>
    ...
    <p class="rb-muted">${text('rollback.resetHelp')}</p>
    ...
    <section class="rb-box"><h2>${text('rollback.stats')}</h2><dl id="rb-stats" class="rb-stats"></dl><p class="rb-muted">${text('rollback.metricsHelp')}</p><p>${text('rollback.history')}</p></section>
    <section class="rb-box"><h2>${text('rollback.timeline')}</h2><p>${text('rollback.legend')}</p><p class="rb-muted">${text('rollback.timelineHelp')}</p><div id="rb-timeline" class="rb-timeline"></div><p id="rb-replay"></p><h3>${text('rollback.select')}</h3><dl id="rb-inspector" class="rb-stats rb-inspector"></dl></section>
    ...
    <article class="explanation">${(['determinism', 'why', 'histories', 'clocks', 'limits'] as const).map(key => `<section><h2>${text(`rollback.${key}`)}</h2><p>${text(`rollback.${key}Help`)}</p>${key === 'histories' ? '<pre><code>stateHistory[N] = state before frame N\nrestore stateHistory[N]\nfor f = N; f &lt; currentFrame; f++:\n  simulate(savedInput[f])\n  save stateHistory[f + 1]</code></pre>' : ''}</section>`).join('')}</article>
  </main>`;
