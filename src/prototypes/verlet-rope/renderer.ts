import { getDistanceConstraintError } from "./constraint";
...
export function renderRope(context: CanvasRenderingContext2D, simulation: RopeSimulation, debug: DebugOptions) {
  ...
  background.addColorStop(0, "#172b4f");
  background.addColorStop(1, "#0e1930");
  context.fillStyle = background;
  context.fillRect(0, 0, canvas.width, canvas.height);
  context.fillStyle = "#273d62";
  context.fillRect(0, simulation.groundY, canvas.width, canvas.height - simulation.groundY);
...
function drawCurrentPoints(context: CanvasRenderingContext2D, simulation: RopeSimulation) {
  simulation.points.forEach((point) => {
    ...
    if (point.isPinned) {
      ...
      context.beginPath();
      context.arc(point.position.x, point.position.y, 4, 0, Math.PI * 2);
      context.fill();
    }
  });
}
...
function drawConstraintLabels(context: CanvasRenderingContext2D, simulation: RopeSimulation) {
  simulation.points.forEach((point, index) => {
    if (index > 0 && index % 3 === 0) {
      ...
      context.fillText(`${index}: ${length.toFixed(1)}`, point.position.x + 7, point.position.y);
    }
  });
}
...
function drawConstraintErrorLabels(context: CanvasRenderingContext2D, simulation: RopeSimulation) {
  simulation.constraints.forEach((constraint, index) => {
    ...
    context.fillText(`Δ ${getDistanceConstraintError(constraint).toFixed(2)}px`, x + 6, y - 5);
  });
}
...
function drawVelocity(context: CanvasRenderingContext2D, x: number, y: number, velocityX: number, velocityY: number) {
  ...
  const endX = x + velocityX * scale;
  const endY = y + velocityY * scale;
  const angle = Math.atan2(endY - y, endX - x);
  context.strokeStyle = "#ff718a";
  context.fillStyle = "#ff718a";
  context.lineWidth = 2;
  context.beginPath();
  context.moveTo(x, y);
