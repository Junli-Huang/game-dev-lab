...
test('horizontal water guard prevents same-tick repeated movement',()=>{
 const w=new World(7,1);w.set(3,0,'water');w.set(2,0,'wood');step(w);assert.equal(at(w,4,0).material,'water');assert.equal(w.stats.movedCells,1);
});
...
test('alternate priority changes by tick; fixed left remains left',()=>{
 for(const [lateral,tick,x] of [['alternate',0,2],['alternate',1,4],['left',1,2]]){
 const w=new World(7,1);w.lateral=lateral;w.tick=tick;w.set(3,0,'water');step(w);assert.equal(at(w,x,0).material,'water');
 }
});
